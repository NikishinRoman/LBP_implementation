...
%% read in videos
% initialize
SaveFolder = 'Replay_LBPTest_singleFrame';
mkdir(SaveFolder)
...
folderReal = 'test/real/';
folderAttackf = 'test/attack/fixed/';  % both contain photo, vid in adverse and controlled
folderAttackh = 'test/attack/hand/';
  ...
  [cs,index] = sort_nat(imgCells,'ascend');
   ...
   for m = 1:length(fileNameList)
       try
            ...
            height = v.Height; 
            frames = read(v);
     load(['../Data/IDAP1stFrameNewTest/' num2str(f) 'Dlib/dLib-' vidName '.png.mat'])
